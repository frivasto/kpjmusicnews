	...
	<footer class="entry-meta entry-footer clearfix">
		<?php
			...
			if ( ! athemes_categorized_blog() ) {
				...
				if ( '' != $tag_list ) {
					$meta_text = __( '<span class="tags-links pull-right"><i class="ico-tags"></i> %2$s</span>', 'athemes' );
				}

			} else {
				// But this blog has loads of categories so we should probably display them here
				if ( '' != $tag_list ) {
					$meta_text = __( '<span class="cat-links pull-right"><i class="ico-folder"></i> %1$s</span><span class="tags-links"><i class="ico-tags"></i> %2$s</span>', 'athemes' );
				}
